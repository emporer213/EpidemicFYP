...
class ContinuousSpace:
    ...
    def place_station(self, station, pos):
        pos = self.torus_adj(pos)
        if self._station_points is None:
            self._station_points = np.array([pos])
        else:
            self._station_points = np.append(self._station_points, np.array([pos]), axis=0)
        self._index_to_station[self._station_points.shape[0] - 1] = station
        self._station_to_index[station] = self._station_points.shape[0] - 1
        station.pos = pos
    ...
    def get_nearest_station(self, pos, radius, nearest_station_rec):
        nearest_station = nearest_station_rec
        deltas = np.abs(self._station_points - np.array(pos))
        if self.torus:
            deltas = np.minimum(deltas, self.size - deltas)
        dists = deltas[:, 0] ** 2 + deltas[:, 1] ** 2

        idxs, = np.where(dists <= radius ** 2)
        neighbors = [self._index_to_station[x] for x in idxs if dists[x] > 0]
        distance_list = []
        if len(neighbors) == 1:
            nearest_station = neighbors[0]
            return nearest_station

        if len(neighbors) > 0:
            for neighbor in neighbors:
                distance_list.append((neighbor, self.get_distance(pos, neighbor.pos)))

            dis_shortest = distance_list[0]
            for i in range(1, len(distance_list)):
                if distance_list[i][1] < dis_shortest[1]:
                    dis_shortest = distance_list[i]

            nearest_station = dis_shortest[0]
            return nearest_station

        radius += 10
        return self.get_nearest_station(pos, radius, nearest_station)
